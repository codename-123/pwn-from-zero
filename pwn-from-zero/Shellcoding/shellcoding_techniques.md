...
## 📌 Shellcode란?
Shellcode는 어셈블리어로 작성된 명령어를 머신 코드로 컴파일한 **실행 가능한 16진수 코드**입니다. 주로 침투 테스트나 취약점 공격에서 메모리에 주입되어 바로 실행됩니다. 이 코드는 리눅스 시스템 콜을 활용해 쉘을 실행하거나 특정 행위를 수행합니다.

---

## ✅ Shellcode가 만족해야 할 조건 (Shellcoding Requirements)
...
## 🛠️ 문제 코드 예시 (Hello World Shellcode)
```nasm
section .data
...
이 코드는 shellcode로 쓰기엔 다음과 같은 문제가 있습니다:
- 변수(message) 사용
- 직접 주소 참조
- NULL 바이트 다수 포함
...
📂 본 문서는 `pwn-from-zero` 가상의 **Shellcoding Techniques** 파트입니다.
