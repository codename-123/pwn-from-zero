...
| 레지스터 | 역할 |
|:---|:---|
| **$rax** | 연산 결과나 함수 리턴값을 저장함. |
| **$rbx** | 보조로 값 저장에 사용함. 특별한 규칙 없음. |
| **$rcx** | 루프(반복문) 횟수 제어에 사용함. |
| **$rdx** | 연산 보조나 시스템 호출(syscall) 보조로 사용함. |
| **$rsp** | 스택 포인터. 스택 최상단(top)을 가리킴. |
| **$rbp** | 베이스 포인터. 함수의 로컬 변수 기준점 역할을 함. |
| **$rsi** | 함수 호출 시 두 번째 인자를 전달함. |
| **$rdi** | 함수 호출 시 첫 번째 인자를 전달함. |
| **$rip** | 명령어 포인터. 다음 실행할 명령어 주소를 가리킴. |
