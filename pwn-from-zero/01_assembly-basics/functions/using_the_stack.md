...
이 문서에서는 이 중 **스택(Stack)** 세그먼트를 다룹니다.

---

## 🧱 스택이란?

**스택(Stack)**은 프로그램이 임시 데이터를 저장하는 **메모리 공간**입니다. 스택의 상단은 `rsp`(Stack Pointer), 하단은 `rbp`(Base Pointer)로 참조합니다.

스택은 **후입선출(LIFO, Last-In First-Out)** 구조이며 다음과 같은 명령으로 조작합니다:
...
## 🛠️ 함수와 시스템 호출에서의 스택 사용

함수나 `syscall`을 호출할 때, **현재 레지스터 상태를 보존하기 위해 push/pop을 사용**합니다. 함수 내부에서 레지스터를 사용할 경우 기존 값이 손실될 수 있기 때문입니다.
...
스택은 복사 구조이므로 `push` 후에도 레지스터 값은 유지됩니다.
...
> 🧠 `push rax -> push rbx -> pop rbx -> pop rax` 순서를 기억하세요!
...
📂 본 문서는 `pwn-from-zero` 프로젝트의 **Using the Stack** 파트입니다.
