...
int main(void) {
	...
	*/
	...
	| 타입          | 바이트 | 주요 사용 목적                | 포너블 포인트                        |
	| ----------- | --- | ----------------------- | ------------------------------ |
	| `char`      | 1   | 문자 1개, 문자열 저장 (`buf[]`) | ✅ 버퍼 오버플로우의 핵심 대상              |
	| `short`     | 2   | 가벼운 숫자, 제한된 범위          | 인접 변수 덮기 쉬움                    |
	| `int`       | 4   | 기본 숫자 처리                | 조건 분기, 권한 체크                   |
	| `long`      | 4 or 8   | 큰 숫자 (32bit or 64bit 기준)         | 구조체 정렬(padding) 영향, 시스템 종속성 존재 |
	| `long long` | 8   | 매우 큰 숫자                 | 포인터 크기와 혼동 주의, 대형 정수 연산 시 사용   |
	...
	*/
	...
	| 타입 | 바이트 | 주요 사용 목적 | 포너블 포인트                               |
	| -------------------- | -- - | ---------------------- | ------------------------------------ - |
	| `unsigned char` | 1 | 0\~255 범위 값(바이트 처리 등) | 바이너리 비교 / 루프 탈출 조건 우회(ex: 0xFF = 255)  |
	| `unsigned short` | 2 | 네트워크 포트, 짧은 정수 처리 | underflow 시 65535 → 조건 우회 가능          |
	| `unsigned int` | 4 | 메모리 크기, 배열 인덱스 등 | 음수 대입 시 → 매우 큰 양수로 변환되어 취약점 유발 가능     |
	| `unsigned long` | 4 or 8 | 32bit or 64bit 시스템에서 포인터 등 처리 | 정수 오버플로우로 out - of - bound 접근 시 사용됨       |
	| `unsigned long long` | 8 | 매우 큰 양의 정수 처리 | 정수 underflow → UAF, OOB, 무한 루프 트리거 가능 |
	...
	*/
	...
	| 타입            | 크기       | 표현 예시          | 범위 (대략)     |
	| ------------- | -------- | -------------- | ----------- |
	| `float`       | 4바이트     | `3.14`, `-2.7` | ±3.4E±38    |
	| `double`      | 8바이트     | `3.1415926535` | ±1.7E±308   |
	| `long double` | 8 or 16바이트 | 시스템마다 다름(double형이나 같거나 큰 범위)       | 더 정밀한 표현 가능 |
